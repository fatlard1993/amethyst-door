#!/usr/bin/env python3
"""Generate Amethyst Door's mod menu icon: the door, in the wall it opens.

Basalt and calcite around amethyst is a geode read from the outside in, which is
what is on the other side of the door. Colours are the vanilla blocks' own.
...
OUT = os.path.join(HERE, "src/main/resources/assets/amethyst-door-justfatlard/icon.png")
...
def build_icon():
    ...
    wall = [row[:] for row in vanilla("block/smooth_basalt.png")[:16]]
    stone = vanilla("block/amethyst_block.png")[:16]

    # A doorway cut through the wall, amethyst in the opening
    for y in range(2, 16):
        for x in range(5, 12):
            wall[y][x] = stone[y][x]
            if x in (5, 11) or y == 2:
                wall[y][x] = light if x == 5 or y == 2 else shadow

    # The panel line and the handle, the two marks that say door rather than hole
    for y in range(5, 14):
        for x in range(7, 10):
            if x in (7, 9) or y in (5, 13):
                wall[y][x] = shadow
    wall[9][10] = light
    return scale(wall, 8)
