...
public final class Pocket {
	...
	public static BlockPos nearestCentre(BlockPos pos) {
		...
		return new BlockPos(x, FLOOR_Y + OUTER_RADIUS, z);
	}

	/** Where a player arriving in their geode should stand: on the floor, facing the door. */
	public static BlockPos arrivalIn(int plot) {
		BlockPos centre = centreOf(plot);

		return new BlockPos(centre.getX(), FLOOR_Y + 1, centre.getZ() + INNER_RADIUS - 3);
	}
	...
	public static BlockPos doorIn(int plot) {
		...
		return new BlockPos(centre.getX(), FLOOR_Y + 1, centre.getZ() + INNER_RADIUS - 1);
	}
